...
enum chips { it87, it8712, it8716, it8718, it8720, it8721, it8728, it8732,
	     ...
	     it8792, it8603, it8620, it8622, it8628, it8616 };
...
/* Thecus IT8616E Patch Start*/

#define BANK_SELE_REG	0x06

/* IT8616E FAN function */
#define FAN2_MOD_CON_REG 0x16		/* The Register of Switch FAN mode for FAN2 */
#define FAN2_SMOOTH_CON_REG 0x6c	/* The Register of Control Smoothing Function for FAN2 */

/* IT8616E EuP function */
#define EUPREG		0xfa		/* Register: EuP Control Register */
#define IT86_EUP_MASK	0x10
#define GET_EUP_WAKEUP_STATUS(val) (((val) & 0x10) >> 4)
#define GET_SYS_WAKEUP_STATUS(val) (((val) & 0x04) >> 2)

/* IT8616E LED function */
#define RED_POLARITY_REG 0xb1		/* Polarity Register of red sys LED */
#define RED_SIMPLEIO_REG 0xc1		/* Simple Enable Register of red sys LED */
#define RED_INPUT_OUTPUT_SELE_REG 0xc9	/* Input/Output Select Register */
#define GPIO_RED_SYS_LED_REG 0x26	/* GPIO Set 2 Multi-Function Pin Selection Register*/
#define BLUE_POLARITY_REG 0xb3		/* Polarity Register of Blue sys LED */
#define BLUE_SIMPLEIO_REG 0xc3		/* Simple Enable Register of Blue sys LED */
#define BLUE_INPUT_OUTPUT_SELE_REG 0xcb	/* Input/Output Select Register of Blue sys LED*/
#define GPIO_BLUE_SYS_LED_REG 0x28	/* GPIO Set 4 Multi-Function Pin Selection Register*/
#define LED_LOCATION_REG 0xf8		/* GP LED Blinking 1 Pin Mapping Register*/
#define LED_BLINK_CTL_REG 0xf9		/* GP LED Blinking 1 Control Register */
#define BLUE_LED_LOCATION 0x22
#define RED_LED_LOCATION 0x12

/* Thecus IT8616E Patch End*/
...
static void it8616_set_blue_led(int val);
static void it8616_set_red_led(int val);
/*
...
static u16 get_device_id(void)
{
	int err;
	u16 chip_type;

	err = superio_enter(REG_2E);
	if (err)
		return err;
	chip_type = force_id ? force_id : superio_inw(REG_2E, DEVID);
	superio_exit(REG_2E);

	return chip_type;
}

static void it8616_set_fan_config(struct it87_data *data)
{
	u8 val;

	/* Switch FAN2 to software opertion */
	val = it87_read_value(data, FAN2_MOD_CON_REG);
	it87_write_value(data, FAN2_MOD_CON_REG, (val & 0x7f));

	/* Disable FAN smoothing function of FAN2 */
	val = it87_read_value(data, FAN2_SMOOTH_CON_REG);
	it87_write_value(data, FAN2_SMOOTH_CON_REG, (val & 0x7f));
}

static int it8616_pme_superio_write(u8 reg, u8 val)
{
	int err;
...
static u8 it8616_pme_superio_read(u8 reg)
{
	int err;
	u8 data;
	...
	if (err)
		return err;
	superio_select(REG_2E, PME);
	data = superio_inb(REG_2E, reg);
	superio_exit(REG_2E);

	return data;
}

static u8 it8616_EUP_status_read(void)
{
	u8 data;
	/* Use 0xfa<1> as the switch of EuP function */
	data = it8616_pme_superio_read(EUPREG);

	return ((data & 0x02) >> 1);
}

static void it8616_EUP_write(u8 val)
{
	u8 ori_val = it8616_pme_superio_read(EUPREG);
	u8 set_val = (ori_val & 0xfd) | (val << 1);
	it8616_pme_superio_write(EUPREG, set_val);
}

static u8 it8616_led_read(u8 reg)
{
	u8 val;
	int err;
	...
	if (err)
		return err;
	superio_select(REG_2E, GPIO);
	val = superio_inb(REG_2E, reg);
	superio_exit(REG_2E);
	return val;
}

static int it8616_led_write(u8 led_reg, u8 led_val)
{
	int err;
	...
	if (err)
		return err;
	superio_select(REG_2E, GPIO);
	superio_outb(REG_2E, led_reg, led_val);
	superio_exit(REG_2E);
	return 0;
}

static void it8616_set_led_conf(void)
{
	/* Switch to GPIO function */
	it8616_led_write(GPIO_RED_SYS_LED_REG, 0xae);
	it8616_led_write(GPIO_BLUE_SYS_LED_REG, 0x04);
	/* Turn off the both LED */
	it8616_led_write(RED_SIMPLEIO_REG, 0x00);
	it8616_led_write(BLUE_SIMPLEIO_REG, 0xff);
	/* Set Polarity Register to inverting mode */
	it8616_led_write(RED_POLARITY_REG, 0x00);
	it8616_led_write(BLUE_POLARITY_REG, 0x00);
	/* RED LED: Set to output mode */
	it8616_led_write(RED_INPUT_OUTPUT_SELE_REG, 0xff);
	it8616_led_write(BLUE_INPUT_OUTPUT_SELE_REG, 0xff);
	/* Initial the location of blinking LED */
	it8616_led_write(LED_LOCATION_REG, 0x00);
	/* Set LED's frequency of blinking function */
	it8616_led_write(LED_BLINK_CTL_REG, 0x00);
}

static void it8616_set_red_led(int val)
{
	if (val == 1) {
		it8616_led_write(RED_SIMPLEIO_REG, 0xff);
	} else
		it8616_led_write(RED_SIMPLEIO_REG, 0x00);
}
...
{
	if (val == 1){
		it8616_led_write(BLUE_SIMPLEIO_REG, 0x00);
	}else{
		it8616_led_write(BLUE_SIMPLEIO_REG, 0xff);
	}
}

static void it8616_set_blink_blue_led(int val)
{
	if (val == 1) {
		/* Set to Alternate function */
		it8616_led_write(BLUE_SIMPLEIO_REG, 0x00);
		/* Set the location to GP42 */
		it8616_led_write(LED_LOCATION_REG, BLUE_LED_LOCATION);
	}else{
		/* Set to simple I/O function */
		it8616_led_write(BLUE_SIMPLEIO_REG, 0x00);
...
static void it8616_set_blink_red_led(int val)
{
	if (val == 1) {
		/* Set to Alternate function */
		it8616_led_write(RED_SIMPLEIO_REG, 0x00);
		/* Set the location to GP42 */
		it8616_led_write(LED_LOCATION_REG, RED_LED_LOCATION);
	}else{
		/* Set to simple I/O function */
		it8616_led_write(RED_SIMPLEIO_REG, 0x00);
		/* Set to default value */
 ...
 * status=1 -> Turn on
 * status=2 -> Blinking */
void it8616_pwr_led_set(int led, int status)
{
	/* Before setting the LED, system need to reset to default value. */
	it8616_set_led_conf();
	switch (led) {
	case 0:
		if(status == 1)
			it8616_set_blue_led(1);
		else if (status == 2)
			it8616_set_blink_blue_led(1);
		else
			it8616_set_blue_led(0);
		break;
	case 1:
		if(status == 1)
			it8616_set_red_led(1);
		else if (status == 2)
			it8616_set_blink_red_led(1);
		else
			it8616_set_red_led(0);
		break;
	default:
		printk("IT8618 LED Contorl: Invalid Value!\n");
		break;
	}
}
EXPORT_SYMBOL(it8616_pwr_led_set);

/*int led = 0 => Blue LED, 1=> Red LED */
int it8616_pwr_led_get_status(int led)
{
	u8 simple_mod_val, blink_led_target_reg;
	u8 blue_simple_mod_val = it8616_led_read(BLUE_SIMPLEIO_REG);
	u8 red_simple_mod_val = it8616_led_read(RED_SIMPLEIO_REG);
        u8 location_val = it8616_led_read(LED_LOCATION_REG);

	/* The default checking setting is Blue LED */
	blink_led_target_reg = BLUE_LED_LOCATION;
	simple_mod_val = blue_simple_mod_val;

	/* If the target LED is Red LED, changing the checking Register */
	if (led == 1){
		blink_led_target_reg = RED_LED_LOCATION;
		/* Because the simple I/O control bit of Blue LED is inverse,
		 * system need to invert the simple I/O value of Red LED,
		 * then it can use the following judgment together. */
		simple_mod_val = ~red_simple_mod_val;
	}

	/* Base on the behavior of Blue LED to tune the checking bit of Red LED
	 * in order to determine the LED status according to the following
	 * uniform judgment */
	if ((simple_mod_val == 0x00) && (location_val == 0x00))
		return 1;
	else if (location_val == blink_led_target_reg)
		return 2;
	else
		return 0;
}
EXPORT_SYMBOL(it8616_pwr_led_get_status);

static void it8616_default_config_set(struct it87_data *data)
{
	/* Fan configure */
	it8616_set_fan_config(data);
	/* LED configure */
	it8616_set_led_conf();
	/* Set Blue LED to blink */
...
static int it87_probe(struct platform_device *pdev)
{
	...
	/* Switch to Bank 0 */
	if ( it87_read_value(data, IT87_REG_CHIPID) != 0x90 )
		it87_write_value(data, BANK_SELE_REG, 0x00);
	...
	if (device_id == IT8616E_DEVID)
		it8616_default_config_set(data);
		...
		size_t length, loff_t *ppos)
{
	...
	if (!strncmp(buffer, "REG", strlen("REG"))) {
		...
		if (i == 1) {
			...
			if (val1==0) {
				printk("ACPWR=%d,Off\n",val1);
				superio_outb(REG_2E, 0xf2,0x00);
				superio_outb(REG_2E, 0xf4,0x00);
			} else if (val1==1) {
				printk("ACPWR=%d,On\n",val1);
				superio_outb(REG_2E, 0xf2,0x00);
				superio_outb(REG_2E, 0xf4,0x20);
			} else {
				printk("ACPWR=%d,Keep last state\n",val1);
				superio_outb(REG_2E,0xf2,0x20);
				superio_outb(REG_2E,0xf4,0x00);
			}
			superio_exit(REG_2E);
			mutex_unlock(&data->update_lock);
		...
		i = sscanf(buffer + strlen("EUP"), "%x\n", &v0);
		/* Only IT8616E support EuP function now */
		if ((i == 1) & (type == IT8616E_DEVID)){
			if (v0 == 0 || v0 == 1) {
				mutex_lock(&data->update_lock);
				it8616_EUP_write(v0);
				mutex_unlock(&data->update_lock);
	...
	} else if (!strncmp(buffer, "SLED", strlen("SLED"))) {
		i = sscanf(buffer + strlen("SLED"), "%d %d %x\n", &v0, &v1, &v2);
		if (v0 == 1){
			printk("LED STATUS = %d\n", it8616_pwr_led_get_status(v1));
		}else if (v0 == 2){
			it8616_pwr_led_set(v1, v2);
			printk("SET %d LED STATUS = %d\n", v1, it8616_pwr_led_get_status(v1));
		}else{
			reg = it8616_led_read(v2);
			printk("The LED status REG = %x\n", reg);
		}
	}
...
const struct proc_ops thecus_proc_it87_operations = {
	.proc_open	= thecus_proc_it87_open,
	.proc_read	= seq_read,
	.proc_write	= thecus_proc_it87_write,
...
static int __init sm_it87_init(void)
{
	...
	if( thecus_it87_init_procfs()){
		printk(KERN_ERR "it87.c: cannot create /proc/hwm .\n");
		return -ENOENT;
	}else{
